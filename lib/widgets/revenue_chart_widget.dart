...
class _RevenueChartWidgetState extends State<RevenueChartWidget>
  ...
  List<FlSpot> _getSpots() {
    ...
    for (int i = 0; i < currentData.length; i++) {
      ...
      // Ensure revenue is never negative and apply animation
      final animatedRevenue = (revenue >= 0 ? revenue : 0) * _animation.value;
      
      // Add the spot
      spots.add(FlSpot(i.toDouble(), animatedRevenue));
    }
  ...
  double _getMaxRevenue() {
    ...
    final positiveRevenue = currentData
        .map((data) => (data['revenue'] as num).toDouble())
        .where((revenue) => revenue >= 0)
        .toList();
    
    if (positiveRevenue.isEmpty) return 100;
    
    final maxRevenue = positiveRevenue.reduce((a, b) => a > b ? a : b);
    return maxRevenue > 0 ? maxRevenue : 100;
  }
